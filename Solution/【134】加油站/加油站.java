...
也就是说diff[i] 也是 >= 0的，这个时候我们还有必要从站点 i + 1尝试吗？
...
　　因此，当我们发现到达k 站点邮箱见底儿后，i 到 k 这些站点都不用作为出发点来试验了，
    ...
    因此解法时间复杂度从O(n2)降到了 O(2n)，是因为将k+1站作为始发站，车得绕圈开回k，来验证k+1是否满足。 
　　等等，真的需要这样吗？ 
　　我们模拟一下过程： 
...
　　要想知道车能否开回 q 站，其实就是在sum3 的基础上，依次加上 diff[0] 到 diff[q]，看看sum3在这个过程中是否会小于0。
    但是我们之前已经知道 diff[0] 到 diff[p-1] 这段路，油箱能一直保持非负，因此我们只要算算sum3 + sum1是否 <0，就知道能不能开到 p+1站了。 
　　如果能从p+1站开出，只要算算sum3 + sum1 + sum2 是否 < 0，就知都能不能开回q站了。 
　　因为 sum1, sum2 都 < 0，因此如果 sum3 + sum1 + sum2 >=0 那么sum3 + sum1 必然 >= 0.
    也就是说，只要sum3 + sum1 + sum2 >=0，车必然能开回q站。而sum3 + sum1 + sum2 其实就是 diff数组的总和 Total，遍历完所有元素已经算出来了。 
　　因此 Total 能否 >= 0，就是是否存在这样的站点的 充分必要条件。 
*/

// 此处都是用到达某个加油站但是未加油的状态
...
class Solution {
    public int canCompleteCircuit(int[] gas, int[] cost) {
        if(gas == null || cost == null || gas.length == 0 || cost.length == 0)
            return -1;
        int start = 0 ;     // 记录出发点
        int total = 0;      // 记录全部遍历的全程的总的 油 - 消耗
        int sum = 0 ;       // 记录当前start 出发的 油 - 消耗
        for(int i = 0 ; i < gas.length ; i ++){
            ...
            if(sum < 0){   // 油箱没有油了
                ...
                sum = sum + (gas[i] - cost[i]);  // 油箱中还要油，更新油箱中的油的量
            }
        }
        if(total < 0)      //  total >= 0 的话 是一定可以找到符合要求的出发点的
            return -1;
        else
            return start ;
    }
